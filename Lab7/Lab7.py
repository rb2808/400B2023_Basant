...
def density_contour(xdata, ydata, nbins_x, nbins_y, ax=None, **contour_kwargs):
    ...
    prime_sigma = so.brentq(find_confidence_interval, 0., 1., args=(pdf, 0.80))


    # Array of Contour levels. Adjust according to the above
    levels = [one_sigma, prime_sigma,two_sigma, three_sigma][::-1]
...
COMD = CenterOfMass("M31_000.txt",2)   # we can do halo or buldge as well. 




# Compute COM of M31 using disk particles                 # we have done before
COMP = COMD.COM_P(0.1)
...
# plt.hist2d(yD,zD, bins= 500, norm=LogNorm(), cmap='magma') 
plt.hist2d(xD,yD, bins= 500, norm=LogNorm(), cmap='viridis') # bins tells you resolution
# bins are counting how many particles are their and calculating number of particles are their.
plt.colorbar()

# so above part gives spiral arms but its hard to pick them up , so add density contour
# ADD HERE
...
density_contour(xD,yD, 80, 80, ax=ax, colors=['cyan','red','yellow' ,'white'])  # we have 3 sigma levels 1,2,3 so we need three colors

# we can add more sigma levels above and corresponding color, so it will pick different spiral arm
...
# this code will compute angular momentum vector for every particle
# and then average over it, so we get normal angular angular momentum , that is normal to our
# disk and then roatate that normal to plane to z axis as we need face on. 
...
plt.hist2d(rn[:,0], rn[:,2], bins = 500, norm =LogNorm(), cmap='viridis') # so we will get something edge on so x and z.
...
plt.hist2d(rn[:,1], rn[:,0], bins = 500, norm =LogNorm(), cmap='viridis') # face on ie ab x and y.
plt.colorbar()
# Add axis labels
plt.xlabel('  ', fontsize=22)
plt.ylabel('  ', fontsize=22)
...
plt.hist2d(rn[:,0],vn[:,1], bins=150,norm=LogNorm())  # this is the rotation curve
plt.colorbar
...
plt.plot(-R, -Vcirc, color= 'red')  # as we should go to negative speed also.
