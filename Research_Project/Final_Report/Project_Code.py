...
### the data from the simulation files (x,y,z,vx,vy,vz) for the given particle type. This code uses the COM code
### developed in class to calculate the new position and velocities of particles relative to the COM. 
...
def specific_ang_mom(filename, ptype):
    ...
    # Calculating the center of mass positions and velocities of the galaxy.
    delta = 0.1
    volDec = 2
    
    COM = CenterOfMass(filename, ptype, volDec)
    ...
    for i in range(len(data[0])):
        ang_x[i] = (data[1][i] - com_pos[0]) * (data[4][i] - com_vel[0]) * mass[i]
        ang_y[i] = (data[2][i] - com_pos[1]) * (data[5][i] - com_vel[1]) * mass[i]
        ang_z[i] = (data[3][i] - com_pos[2]) * (data[6][i] - com_vel[2]) * mass[i]
...
### Here I am calculating the time-average ASAM of the remnant post merger.
av_rem_x = []
av_rem_y = []
av_rem_z = []

# This loop tracks the remnant ASAM post merger. 
for i in range(len(t)):
    if t[i] > t3:
        av_rem_x.append(rem_x[i])
        av_rem_y.append(rem_y[i])
        av_rem_z.append(rem_z[i])
        
#Printing the avaerage ASAM with uncertainties. 
print('Average remnant ASAM X:', np.average(av_rem_x), 'with deviation:', np.std(av_rem_x))
print('Average remnant ASAM Y:', np.average(av_rem_y), 'with deviation:', np.std(av_rem_y))
print('Average remnant ASAM Z:', np.average(av_rem_z), 'with deviation:', np.std(av_rem_z))

# Printing initial ASAM of MW
print('Initial ASAM MW, X:', Lxmw[0])
print('Initial ASAM MW, Y:', Lymw[0])
print('Initial ASAM MW, Z:', Lzmw[0])

# Printing initial ASAM of M31
print('Initial ASAM M31, X:', Lxm31[0])
print('Initial ASAM M31, Y:', Lym31[0])
print('Initial ASAM M31, Z:', Lzm31[0])
...
fig.suptitle("Specific Angular Momentum Evolution\n of Milky Way "
             + 'and M31 Halos', fontsize = 26)
axs[0].plot(t, Lxmw, color = 'b', label='$\hat{x}$')
axs[0].plot(t, Lymw, color = 'r', label='$\hat{y}$')
axs[0].plot(t, Lzmw, color = 'g', label='$\hat{z}$')
axs[0].plot(t, L_mag_mw, '--', markersize = 2, color = 'navy', label = 'Mag')
axs[0].plot(xax_x, xax_y, color = 'k', linewidth = 1)
axs[0].set_ylabel('MW [kpc-km-$s^{-1}$]', fontsize = 20)
axs[0].tick_params(axis='both', labelsize=15)
axs[0].text(3.1, 9800, '3.957 \n Gyrs', fontsize = 14)
axs[0].text(5.0, 9800, '5.857 \n Gyrs', fontsize = 14)
axs[0].text(6.7, 9800, '6.486 \n Gyrs', fontsize = 14)
axs[0].text(3.1, 9800, '3.957 \n Gyrs', fontsize = 14)
axs[0].text(5.0, 9800, '5.857 \n Gyrs', fontsize = 14)
axs[0].text(6.7, 9800, '6.486 \n Gyrs', fontsize = 14)
...
axs[1].plot(t, L_mag_m31, '--', markersize = 2, color = 'navy', label = 'Mag')
axs[1].plot(xax_x, xax_y, color = 'k', linewidth = 1)
axs[1].set_xlabel('Time (Gyrs)', fontsize = 15)
axs[1].set_ylabel('M31 [kpc-km-$s^{-1}$]', fontsize = 20)
axs[1].tick_params(axis='both', labelsize=15)
axs[1].text(3.1, 8800, '3.957 \n Gyrs', fontsize = 14)
axs[1].text(5.0, 8800, '5.857 \n Gyrs', fontsize = 14)
axs[1].text(6.7, 8800, '6.486 \n Gyrs', fontsize = 14)
axs[1].text(3.1, 8800, '3.957 \n Gyrs', fontsize = 14)
axs[1].text(5.0, 8800, '5.857 \n Gyrs', fontsize = 14)
axs[1].text(6.7, 8800, '6.486 \n Gyrs', fontsize = 14)
...
mwlx = [min(Lymw), max(Lymw)]
m31lx = [min(Lym31), max(Lym31)]
lx1time = [3.957, 3.957]
lx2time = [5.857, 5.857]
lx3time = [6.486, 6.486]
axs[0].plot(lx1time, mwlx, '--', color = 'm', linewidth = 2)
...
fig.suptitle("Evolution of Halo's Specific Angular Momentum\n [Pre-Me"
             +'rger Galaxies (MW & M31) and Post-Merger Remnant]', fontsize = 26)
axs[0].plot(t, Lxmw, color = 'b', label= '$\hat{x}$')
axs[0].plot(t, Lymw, color = 'r', label= '$\hat{y}$')
axs[0].plot(t, Lzmw, color = 'g', label= '$\hat{z}$')
...
axs[2].set_ylabel('Remnant \n [kpc-km-$s^{-1}$]', fontsize = 15)
axs[2].tick_params(axis='both', labelsize=15)
axs[2].set_xlabel('Time (Gyrs)', fontsize = 14)
