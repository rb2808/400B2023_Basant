...
# of MW and M31
time_mw_m31, dist_mw_m31, vel_mw_m31 = dist_vel(MW_data, M31_data)

# of M33 and M31
time_m33_m31, dist_m33_m31, vel_m33_m31 = dist_vel(M33_data, M31_data)


### The next lines are for the research project which finds out the time of minimum 
### approaches (first, second, and then when they finally merge). This is done for
### mw and m31 becoz m33 seems weird and is more like a satellite. 
use_t = []

for j in range(len(time_mw_m31)):
    if 3.5 < time_mw_m31[j] < 5:
        use_t.append(j)

first = []

for k in range(len(use_t)):
    first.append(float(dist_mw_m31[use_t[k]]))


print(time_mw_m31[use_t[first.index(min(first))]])


use_t = []

for j in range(len(time_mw_m31)):
    if 5 < time_mw_m31[j] < 6:
        use_t.append(j)
...
# Plot the Orbit of the galaxies. Here we plot the relative distance and velocity for both MW and M31 and
# M31 and M33 as a function of time.  
#################################

# Distance between MW and M31. 
plt.figure(figsize = (10, 10))
plt.plot(time_mw_m31, dist_mw_m31, label = 'Distance MW - M31', color = 'b')
plt.title('Distance between Milky Way and M31', fontsize = 25)
plt.xlabel('Time [Gyrs]', fontsize = 20)
...
#################################
'''
# Relatice velocity between MW and M31. 
plt.figure(figsize = (10, 10))
plt.plot(time_mw_m31, vel_mw_m31, label = 'Rel. Velocity MW - M31', color = 'r')
plt.title('Relative Velocity between Milky Way and M31', fontsize = 25)
plt.xlabel('Time [Gyrs]', fontsize = 20)
